...
public interface UserHierarcyRepository extends JpaRepository<UserHierarchy, Long> {
	...
	@Query("select distinct  h from UserHierarchy h where h.hrManager.id = :hrManager and h.orgHead.id = :orgHead")
	List<UserHierarchy> findByHrId(@Param("hrManager") long hrManager, @Param("orgHead") long orgHead);
	...
	@Query("select distinct  h from UserHierarchy h where h.orgHead.id = :orgHead and h.groupHead.id = :groupHead")
	List<UserHierarchy> findByOrgHeadAndGropHead(@Param("orgHead") long orgHead, @Param("groupHead") long groupHead);
	
	@Query("select distinct  h from UserHierarchy h where h.orgHead.id = :orgHead and "
			+ "h.hrManager.id = :hrManager")
	List<UserHierarchy> findByHrAndOrgHead(@Param("orgHead") long orgHead,
											@Param("hrManager") long hrManager);
	
	@Query("select distinct  h from UserHierarchy h where h.orgHead.id = :orgHead and "
			+ "h.hrManager.id = :hrManager and "
			+ "h.priSupervisor.id = :priSupervisor")
	List<UserHierarchy> findByHrAndOrgHeadAndPriSuper(@Param("orgHead") long orgHead, 
														@Param("hrManager") long hrManager,
														@Param("priSupervisor") long priSupervisor);
	
	@Query("select distinct  h from UserHierarchy h where h.orgHead.id = :orgHead and"
					+ " h.hrManager.id = :hrManager and "
					+ "h.priSupervisor.id = :priSupervisor and "
					+ "h.prjSupervisor.id = :prjSupervisor")
	List<UserHierarchy> findByHrAndOrgHeadAndPriSuperAndPrjSuper(@Param("orgHead") long orgHead, 
																	@Param("hrManager") long hrManager, 
																	@Param("priSupervisor") long priSupervisor, 
																	@Param("prjSupervisor") long prjSupervisor);
	
	@Query("select distinct  h from UserHierarchy h where h.orgHead.id = :orgHead and"
			+ " h.hrManager.id = :hrManager and "
			+ "h.priSupervisor.id = :priSupervisor and "
			+ "h.prjSupervisor.id = :prjSupervisor and "
			+ "h.user.id = :user")
	List<UserHierarchy> findByHrAndOrgHeadAndPriSuperAndPrjSuperAndUser(@Param("orgHead") long orgHead, 
															@Param("hrManager") long hrManager, 
															@Param("priSupervisor") long priSupervisor, 
															@Param("prjSupervisor") long prjSupervisor,
